...
class Attribute {
	...
	immutable int index;
	immutable uint location;
	immutable string name;
	immutable GLenum type;

	final void Set(vec4 v) const {
		glUniform4f(location, v.x,v.y,v.z,v.w);
	}

	final void Set(vec3 v) const {
		glUniform3f(location, v.x,v.y,v.z);
	}

	final void Set(vec2 v) const {
		glUniform2f(location, v.x,v.y);
	}

	final void Set(float v) const {
		glUniform1f(location, v);
	}

	final void Set(vec4d v) const {
		glUniform4d(location, v.x,v.y,v.z,v.w);
	}

	final void Set(vec3d v) const {
		glUniform3d(location, v.x,v.y,v.z);
	}

	final void Set(vec2d v) const {
		glUniform2d(location, v.x,v.y);
	}

	final void Set(double v) const {
		glUniform1d(location, v);
	}

	final void Set(vec4i v) const {
		glUniform4i(location, v.x,v.y,v.z,v.w);
	}

	final void Set(vec3i v) const {
		glUniform3i(location, v.x,v.y,v.z);
	}

	final void Set(vec2i v) const {
		glUniform2i(location, v.x,v.y);
	}

	final void Set(int v) const {
		glUniform1i(location, v);
	}

	final void Set(bool v) const {
		glUniform1i(location, cast(bool)v);
	}

	final void Set(ref mat4 m) const {
		glUniformMatrix4fv(location, 1, GL_TRUE, m.value_ptr);
	}

	final void Set(mat4 m) const {
		glUniformMatrix4fv(location, 1, GL_TRUE, m.value_ptr);
	}
}
